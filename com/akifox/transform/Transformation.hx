...
class Transformation extends EventDispatcher
{
  ...
  **/
  ...
  private function set_matrix(m:Matrix):Matrix {
  ...
  * Class instance
  *
  * All the parameters are optional and you don't need to exclicit them if you plan
  * to bind the transformation to a DisplayObject (they will be inherited at the binding process)
  *
  * @param matrix   The original matrix (optional)
  * @param width    The rectangle width (optional)
  * @param height   The rectangle height (optional)
  * @param pivot    An absolute point to set the pivot (optional)
  **/
  ...
  /**
  * Bind the transformation matrix to a target (has to be a DisplayObject)
  **/
  ...
  * ----
  *
  * Otherwise you can set the size to arbitrary values if you want to
  * set a certain AnchoredPivot based on the rect you specify
  *
  **/
  ...
  **/
  public function debugDraw(  drawPivot:Bool=true,
    ...
    if (drawBoundaries) {
      ...
      _width = Math.max(Math.max(Math.max(p0.x,p1.x),p2.x),p3.x)-_identityZeroX;
      _height = Math.max(Math.max(Math.max(p0.y,p1.y),p2.y),p3.y)-_identityZeroY;
  ...
  **/
  ...
  //private function set_width(value:Float):Float { return _width; }

  public var height(get, never):Float;
  private function get_height():Float { return _height; }
  //private function set_height(value:Float):Float { return _height; }
  ...
  **/
  ...
  {
    point.x = 0; point.y = 1;
    point = deltaTransformPoint(point);
